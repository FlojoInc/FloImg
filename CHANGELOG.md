...
- fix: Add crossOrigin to InputNode for authenticated cross-origin image requests
  - Enables CORS credentials for deployments where UI and API are on different subdomains
- refactor: InputNode now requires StorageAdapter
  - Removed legacy upload fallback code
  - Removed UploadGallery component and unused upload functions from api/client.ts
...
- fix: Skip cache for runner stage to ensure build args are embedded
  - GHA cache backend doesn't invalidate on build-arg value changes
  - Added `no-cache-filters: runner` to force rebuild of production stage
...
- fix: Inherit build args in production stage for deployment visibility
  - (Note: This fix was incomplete due to Docker layer caching - see v0.21.12)
...
- feat: Add ValidationIssuesPanel to NodeInspector
  ...
  - Separate styling for errors (red) and warnings (amber)
- feat: Auto-select error node when validation fails for immediate visibility
...
- feat: Add optional callbacks to AIChat component for generation events
  - `onGenerationSuccess(data)` - called when workflow generation succeeds
  - `onGenerationFailed(data)` - called when workflow generation fails
  - Useful for tracking, logging, or custom UI feedback
...
- fix: Include floimg-vectorize COPY instructions in Dockerfile
  - Fixes Docker build failure "Cannot find module '@teamflojo/floimg-vectorize'"
  - Adds package.json and source copies in builder stage
  - Adds package.json and dist copies in runner stage

## [v0.21.1] - 2026-01-31

### Build Fixes

- fix: Add floimg-vectorize to npm publish workflow
- fix: Add floimg-vectorize to Docker build steps
...
- feat: Add parameter validation API for pipelines and templates
  ...
  - `validateStep()` - Validate single step
  - `validateGeneratorParams()` - Validate generator parameters
  - `validateTransformParams()` - Validate transform parameters
  - Detects missing required params, unknown params (warns), type mismatches
  - Validates enum values, number min/max constraints
...
- fix: Update to floimg-studio-shared@0.9.1 with composite transform fixes
